...
Envoy reverse-proxy configs for putting an L7 edge in front of the static export origin (typically nginx from `infrastructure/server/nginx` or `infrastructure/global/docker`).
...
docker run --rm -p 8080:8080 -p 9901:9901 \
  ...
  -v "$PWD/infrastructure/server/proxy/envoy.yaml:/etc/envoy/envoy.yaml:ro" \
