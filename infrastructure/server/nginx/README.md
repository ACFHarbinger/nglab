...
Standalone nginx configs for serving the Next.js static export (`out/`), as an alternative to the docker-compose stack under `infrastructure/global/docker/`.
...
docker run --rm -p 8080:80 \
  ...
  -v "$PWD/infrastructure/server/nginx/nginx.conf:/etc/nginx/nginx.conf:ro" \
  -v "$PWD/infrastructure/server/nginx/conf.d:/etc/nginx/conf.d:ro" \
  nginx:1.27-alpine
```
