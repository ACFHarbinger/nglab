...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        if self.method == 'ARP' or embed_dim is not None:
            self.W_out = nn.Parameter(torch.Tensor(n_heads, key_dim, embed_dim))
        elif self.method == 'HADRL':
            # pickup
    ...
    def forward(self, q, h=None, mask=None):
        ...
        if self.method == 'HADRL':
            ...
            # torch.zeros(self.n_heads, batch_size, n_pick, self.input_dim // self.n_heads, dtype=V.dtype, device=V.device)], 2)        
            V_additional_pick = torch.cat([  # [n_heads, batch_size, graph_size, key_size]
                torch.zeros(self.n_heads, batch_size, 1, self.input_dim // self.n_heads, dtype=V.dtype, device=V.device),
                torch.zeros(self.n_heads, batch_size, n_pick, self.input_dim // self.n_heads, dtype=V.dtype, device=V.device),
...
class GraphAttentionEncoder(nn.Module):
    def __init__(
        ...
        self.layers = nn.Sequential(*(
            MultiHeadAttentionLayer(method, n_heads, embed_dim, feed_forward_hidden, normalization)
