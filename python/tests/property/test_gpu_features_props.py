...
def test_sma_properties(gpu_engineer, prices, window):
    ...
    sma_tensor = gpu_engineer.moving_average(torch.tensor(prices, dtype=torch.float32), window)
    ...
    np.testing.assert_allclose(sma_result[window-1:], expected_sma[window-1:], rtol=1e-5, atol=1e-5)
