...
class TestGymLoop:
    ...
    def test_zero_copy_check(self):
        ...
        env = nglab.TradingEnv(initial_capital=10000.0, lookback=50, max_steps=100, enable_logging=False)
        obs, _ = env.reset()
        ...
        assert isinstance(obs, np.ndarray)
        assert obs.flags['OWNDATA'] # If it owns data, it was copied. If False, it's a view.
