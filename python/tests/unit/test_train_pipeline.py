...
@patch("torch.save")
def test_train_epoch(mock_torch_save, mock_get_inner, mock_log_epoch, mock_train_batch, dummy_model, dummy_dataset, tmp_path):
    optimizer = torch.optim.Adam(dummy_model.parameters())
    baseline = MagicMock() # Can be mock now if we don't actually save
    scheduler = MagicMock()
    tb_logger = MagicMock()
    opts = {
        "run_name": "test",
    ...
    train_epoch(dummy_model, optimizer, baseline, scheduler, 0, dummy_dataset, tb_logger, opts)
