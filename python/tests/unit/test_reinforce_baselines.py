...
def test_rollout_baseline_init():
    ...
    with patch("python.src.pipeline.core.reinforce.reinforce_baselines.rollout", return_value=torch.ones(100)), \
         patch("python.src.pipeline.core.reinforce.reinforce_baselines.copy.deepcopy", side_effect=lambda x: x):
...
def test_rollout_baseline_epoch_callback():
    ...
    with patch("python.src.pipeline.core.reinforce.reinforce_baselines.rollout", return_value=torch.ones(100) * 10), \
         patch("python.src.pipeline.core.reinforce.reinforce_baselines.copy.deepcopy", side_effect=lambda x: x):
        baseline = RolloutBaseline(mock_model, mock_problem, mock_opts)
        baseline.mean = 10.0
        ...
        with patch("python.src.pipeline.core.reinforce.reinforce_baselines.rollout", return_value=torch.ones(100) * 5), \
             patch("python.src.pipeline.core.reinforce.reinforce_baselines.ttest_rel") as mock_ttest:
