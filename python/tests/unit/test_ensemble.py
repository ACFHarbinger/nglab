...
def test_predict_with_uncertainty():
    ...
    assert torch.allclose(res["std"], torch.ones(2, 5) * torch.tensor([1, 3], dtype=torch.float32).std())
...
def test_create_ensemble_from_configs(mock_backbone):
    ...
    ensemble = create_ensemble_from_configs(configs, strategy="weighted", weights=[0.4, 0.6])
