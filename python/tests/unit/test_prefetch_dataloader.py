...
def test_cudaprefetcher_to_device():
    with patch("torch.cuda.Stream"):
        prefetcher = CUDAPrefetcher(MagicMock(), device="cpu") # Use cpu for simple test
...
@patch("torch.cuda.Stream")
def test_cudaprefetcher_iter_next(mock_stream_class, mock_stream_context, mock_current_stream, dataloader):
...
def test_background_prefetcher_exception(dataset):
    class FailingDataset(Dataset):
        ...
        def __getitem__(self, idx): raise ValueError("Test Error")
...
def test_prefetch_dataloader_init(dataset):
    ...
    with patch("torch.cuda.is_available", return_value=True):
        dl_cuda = PrefetchDataLoader(dataset, batch_size=2, device="cuda:0", num_workers=2)
...
def test_prefetch_dataloader_iter(dataset):
    ...
    # CUDA case
    with patch("torch.cuda.is_available", return_value=True):
        with patch("python.src.data.prefetch_dataloader.CUDAPrefetcher") as mock_prefetcher:
