...
def mock_boto3():
    mock = MagicMock()
    with patch.dict(sys.modules, {"boto3": mock, "botocore": MagicMock(), "botocore.config": MagicMock()}):
        yield mock
...
class TestS3Storage:
    ...
    def test_load_model(self, s3_storage, mock_boto3):
        ...
        # Mock response body
        body_mock = MagicMock()
        body_mock.read.return_value = b"compressed_data" # Assuming compression logic handles this or dummy
