...
class DCIGN(nn.Module):
    ...
    def __init__(self, input_dim, latent_dim=128, hidden_channels=[32, 64, 128, 256], num_intrinsic=32, num_extrinsic=96, output_type='prediction'):
        ...
        for h_channels in hidden_channels:
            enc_layers.append(nn.Conv1d(last_channels, h_channels, kernel_size=4, stride=2, padding=1))
        ...
        for h_channels in reversed(hidden_channels):
            dec_layers.append(nn.ConvTranspose1d(last_channels, h_channels, kernel_size=4, stride=2, padding=1))
