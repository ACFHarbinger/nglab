...
class PCRModel(DimReductionModel):
    ...
    def fit(self, X, y=None):
        if hasattr(X, "numpy"): X = X.cpu().numpy()
        self.pca.fit(X)
        X_pca = self.pca.transform(X)
        if y is not None:
             if hasattr(y, "numpy"): y = y.cpu().numpy()
...
class PLSRModel(DimReductionModel):
    ...
    def fit(self, X, y):
        ...
        if X.ndim == 3: X = X.reshape(X.shape[0]*X.shape[1], -1)
        if y is not None and y.ndim > 1: y = y.reshape(-1, y.shape[-1]) if y.ndim>1 else y.ravel()
        elif y is not None: y = y.ravel()
...
class QDAModel(DimReductionModel):
    ...
    def forward(self, x, **kwargs):
        ...
        if not self._is_fitted: return torch.zeros((x.shape[0], 1)).to(x.device)
        if hasattr(x, "numpy"): x = x.cpu().numpy()
        if x.ndim==3: x = x[:,-1,:]
...
class FDAModel(DimReductionModel):
    ...
    def forward(self, x, return_embedding=None, **kwargs):
        ...
        if not self._is_fitted: return torch.zeros((x.shape[0], self.n_components or 1)).to(x.device)
