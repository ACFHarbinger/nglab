...
class mLSTMCell(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_heads=4):
        ...
        assert self.head_dim * num_heads == hidden_dim, "Hidden dim must be divisible by num_heads"
    ...
    def forward(self, x, state):
        ...
        if state is None:
            # Init state
            dev = x.device
            C_prev = torch.zeros(batch_size, self.num_heads, self.head_dim, self.head_dim, device=dev)
