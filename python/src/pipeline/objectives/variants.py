...
class SAPOLoss(ClipPPOLoss):
    ...
    def forward(self, tensordict: TensorDict) -> TensorDict:
        ...
        # Adaptive Tau
        tau = torch.where(advantage > 0, torch.tensor(self.tau_pos, device=advantage.device), torch.tensor(self.tau_neg, device=advantage.device))
        ...
        return TensorDict({
            ...
            "loss_entropy": -self.entropy_bonus * entropy if self.entropy_bonus else torch.tensor(0.0),
        }, batch_size=[])
