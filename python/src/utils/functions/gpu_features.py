...
class GPUFeatureEngineer:
    ...
    def _to_tensor(self, data: Union[torch.Tensor, list]) -> torch.Tensor:
    ...
    def macd(self, data: torch.Tensor, fast: int = 12, slow: int = 26, signal: int = 9) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
    ...
    def bollinger_bands(self, data: torch.Tensor, window: int = 20, num_std: float = 2.0) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
