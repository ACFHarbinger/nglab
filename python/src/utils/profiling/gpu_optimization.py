...
class GPUMemoryOptimizer:
    ...
    def compare_snapshots(
        ...
        """
        before = next(
            (s for s in self._snapshots if s.get("label") == before_label),
            None
        )
        after = next(
            (s for s in self._snapshots if s.get("label") == after_label),
            None
        )
...
def get_gpu_optimization_recommendations() -> List[str]:
    ...
    if capability[0] >= 8:  # Ampere or newer
        recommendations.append("✅ GPU supports BF16 - use bf16-mixed precision for best performance")
        recommendations.append("✅ GPU supports Flash Attention - enable via enable_memory_efficient_attention()")
