...
vi.mock("../../../components/terminal/MarketSidebar", () => ({
  MarketSidebar: (props: any) => (
    <div
      ...
      data-has-favorites={!!props.favoriteIds}
      data-has-toggle={!!props.onToggleFavorite}
    >
      Sidebar
    </div>
  ),
...
vi.mock("../../../components/terminal/TradingFormWidget", () => ({
  TradingFormWidget: (props: any) => (
    <div
      ...
      data-has-outcomes={!!props.outcomes}
      data-has-prices={!!props.livePrices}
    >
      TradingForm
    </div>
  ),
...
describe("TerminalLayout", () => {
  ...
  it("should fetch trending markets on mount", async () => {
    const mockMarkets = [
      { id: "1", title: "Market 1", markets: [{ id: "1", title: "Market 1" }] },
    ];
    ...
    await waitFor(() => {
      expect(mockInvoke).toHaveBeenCalledWith("get_trending_markets", {
        limit: 10,
      });
    });
  });
