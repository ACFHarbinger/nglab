...
export function OrderBook({ book }: OrderBookProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 p-4 rounded-lg border border-gray-800">
      <h2 className="text-sm text-gray-400 mb-2 uppercase tracking-wide font-semibold">
        Order Book
      </h2>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {bids.map((level) => (
              <div
                key={level.price}
                className="relative flex justify-between items-center py-0.5 px-1 hover:bg-gray-900 group"
              >
                <span className="z-10 text-gray-300">
                  {level.total_quantity.toFixed(2)}
                </span>
                <span className="z-10 text-green-400 font-medium">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {asks.map((level) => (
              <div
                key={level.price}
                className="relative flex justify-between items-center py-0.5 px-1 hover:bg-gray-900"
              >
                <span className="z-10 text-red-400 font-medium">
                  {level.price.toFixed(2)}
                </span>
                <span className="z-10 text-gray-300">
                  {level.total_quantity.toFixed(2)}
