...
const MARKET_ACTIVITY = [
    { id: '1', bid: 'M', market: 'XRP Up or Down - January 17, 5:45AM-6:...', price: '10¢', amount: '$1.00', color: 'text-emerald-400' },
    { id: '2', bid: 'N', market: 'Ethereum Up or Down - January 17, 5:45...', price: '65¢', amount: '$3.25', color: 'text-rose-400' },
    { id: '3', bid: 'M', market: 'Will the price of XRP be between $2.10...', price: '99¢', amount: '$40.00', color: 'text-emerald-400' },
    { id: '4', bid: 'N', market: 'Bitcoin Up or Down - January 17, 6:00A...', price: '45¢', amount: '$2.25', color: 'text-rose-400' },
    { id: '5', bid: 'Y', market: 'Will the price of Bitcoin be above $96...', price: '1¢', amount: '$0.00', color: 'text-yellow-400' },
    { id: '6', bid: 'Y', market: 'Will the price of Bitcoin be above $96...', price: '1¢', amount: '$0.00', color: 'text-yellow-400' },
    { id: '7', bid: 'Y', market: 'Will the price of Bitcoin be above $96...', price: '1¢', amount: '$0.00', color: 'text-yellow-400' },
    { id: '8', bid: 'Y', market: 'Will the price of Bitcoin be above $96...', price: '1¢', amount: '$0.00', color: 'text-yellow-400' },
    { id: '9', bid: 'Y', market: 'Will the price of Bitcoin be above $96...', price: '1¢', amount: '$0.00', color: 'text-yellow-400' },
];

const NEWS_ITEMS = [
    {
        id: 1,
        title: "PLA Eastern Theatre Command Tracks US Naval Assets - Regional Tensions Rise",
        description: "The People's Liberation Army Eastern Theatre Command has deployed naval and air forces to monitor U.S. vessel movements in the region, indicating...",
        source: "Twitter",
        sentiment: "BULLISH",
    ...
    {
        id: 2,
        title: "US Naval Transit Through Taiwan Strait Heightens Regional Military Tensions",
        description: "Chinese military confirmed two US vessels - USS John Finn (destroyer) and USNS Mary Sears (survey ship) - conducted a freedom of navigation...",
        source: "Twitter",
        sentiment: "BULLISH",
        sentimentConf: "152 conf",
        time: "+5m ago",
            ...
            <div className="flex flex-col">
                ...
                <div className="max-h-[250px] overflow-y-auto custom-scrollbar">
                    ...
                    {MARKET_ACTIVITY.map((item, i) => (
                        <div key={i} className="flex items-center px-2 py-1.5 border-b border-slate-800/30 hover:bg-slate-800/40 transition-colors cursor-pointer group">
                            <span className={clsx("w-6 text-[10px] font-bold", item.color)}>{item.bid}</span>
                            <span className="flex-1 text-[10px] text-slate-400 group-hover:text-slate-200 truncate pr-2">{item.market}</span>
                            <span className="w-12 text-[10px] text-slate-300 text-right font-mono">{item.price}</span>
                            <span className="w-14 text-[10px] text-slate-500 text-right font-mono">{item.amount}</span>
                        </div>
                    ))}
                </div>
            ...
            <div className="flex-1 flex flex-col min-h-0">
                ...
                <div className="flex items-center border-b border-slate-800 bg-slate-950">
                    {INTEL_TABS.map(tab => (
                        ...
                        >
                            ...
                            {tab}
                            {tab === 'OSINT' && (
                                <span className="bg-rose-500 text-white text-[8px] px-1 rounded-full ml-1">1</span>
                            )}
                        </button>
                    ))}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    <div className="flex flex-col gap-2 p-2">
                        {NEWS_ITEMS.map(news => (
                            <div key={news.id} className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-3 hover:border-indigo-500/30 transition-all cursor-pointer">
                                {/* Header */}
                                <div className="flex items-center gap-2 mb-2">
                                    <div className="w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center">
                                        <Radio size={12} className="text-indigo-400" />
                                    </div>
                                    <h4 className="flex-1 text-[11px] font-semibold text-slate-200 leading-tight">{news.title}</h4>
                                    <span className="text-[9px] text-slate-500 whitespace-nowrap">{news.time}</span>
                                </div>
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className={clsx(
                                        ...
                                        news.sentiment === 'BULLISH' ? "bg-emerald-900/30 text-emerald-400" : "bg-rose-900/30 text-rose-400"
                                    )}>
                                        {news.sentiment}
                                    </span>
                                    <span className="text-[8px] text-slate-500">{news.sentimentConf}</span>
                                </div>
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <button className="flex items-center gap-1 text-[9px] text-indigo-400 hover:text-indigo-300 transition-colors">
                                            <ExternalLink size={10} /> {news.relatedMarkets} related markets
                                        </button>
