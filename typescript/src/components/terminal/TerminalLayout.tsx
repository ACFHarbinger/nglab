...
const generateMockMarkets = () => [
    { id: '1', symbol: 'TRUMP', name: 'Trump 2024 Election Win', price: 0.52, change24h: 2.5, volume24h: 154000, isFavorite: true },
    { id: '2', symbol: 'Biden', name: 'Biden 2024 Election Win', price: 0.12, change24h: -5.1, volume24h: 45000 },
    { id: '3', symbol: 'ETH', name: 'ETH > $3k by March', price: 0.78, change24h: 1.2, volume24h: 89000 },
    { id: '4', symbol: 'FED', name: 'Fed Rate Cut in May', price: 0.45, change24h: 0.0, volume24h: 22000 },
    { id: '5', symbol: 'BTC', name: 'BTC > $100k in 2024', price: 0.33, change24h: -1.5, volume24h: 67000, isFavorite: true },
];
...
export function TerminalLayout() {
    ...
    useEffect(() => {
        ...
        if (livePrices[selectedMarketId]) {
            setChartData(prev => [
                ...prev,
                { time: Math.floor(Date.now() / 1000), value: livePrices[selectedMarketId] }
            ]);
        }
    ...
    return (
        <div className="flex h-full w-full bg-slate-950 text-slate-200 overflow-hidden">
                ...
                <header className="flex justify-between items-center px-4 py-3 border-b border-slate-800 bg-slate-950">
                    <div className="flex items-center gap-4">
                        <h1 className="text-xl font-bold flex items-center gap-2">
                            {selectedMarket.symbol} <span className="text-slate-500 font-normal text-sm">/ USD</span>
                        </h1>
                        <div className="flex flex-col">
                            <span className="text-2xl font-mono font-bold text-white">${currentPrice.toFixed(3)}</span>
                            <span className={`text-xs ${selectedMarket.change24h >= 0 ? 'text-green-400' : 'text-rose-400'}`}>
            ...
            <div className="w-80">
                <TradingFormWidget symbol={selectedMarket.symbol} currentPrice={currentPrice} />
            </div>
        </div>
    );
}
