    ...
    return (
        <div className="flex flex-col h-full bg-slate-900 border-l border-slate-800 w-72">
            <div className="p-3 border-b border-slate-800">
                <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Order Book</h3>
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden text-xs font-mono">
                ...
                <div className="flex-1 overflow-y-auto flex flex-col-reverse custom-scrollbar">
                    {sortedAsks.map((ask) => (
                        <div key={ask.price} className="relative flex justify-between items-center py-0.5 px-2 hover:bg-slate-800/50">
                            {/* Depth Bar */}
                            <div
                            ...
                            <span className="z-10 text-slate-300">{ask.total_quantity.toFixed(0)}</span>
                            <span className="z-10 text-slate-500">{(ask.price * ask.total_quantity).toFixed(0)}</span>
                        </div>
                    ))}
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    {sortedBids.map((bid) => (
                        <div key={bid.price} className="relative flex justify-between items-center py-0.5 px-2 hover:bg-slate-800/50">
                            {/* Depth Bar */}
                            <div
                                className="absolute right-0 top-0 bottom-0 bg-emerald-900/20"
                                style={{ width: `${(bid.total_quantity / maxVol) * 100}%` }}
                            />
                            <span className="z-10 text-emerald-400">{bid.price.toFixed(3)}</span>
                            <span className="z-10 text-slate-300">{bid.total_quantity.toFixed(0)}</span>
                            <span className="z-10 text-slate-500">{(bid.price * bid.total_quantity).toFixed(0)}</span>
                        </div>
                    ))}
                </div>
            </div>
