...
function AnalysisTab({
  ...
  const [chartType, setChartType] = useState<
    "price" | "spread" | "volatility" | "candlestick" | "heatmap"
  >("price");
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "📊 Fetching recent historical data to initialize live mode...",
      );
      ...
      const initializeHistory = async () => {
        try {
          ...
          console.log(
            `Fetching historical data for ${activeMarket.outcomes.length} outcomes...`,
          );
          ...
          const fetchPromises = activeMarket.outcomes.map(async (outcome) => {
            try {
              ...
              if (!response.ok) {
                console.warn(
                  `Failed to fetch ${outcome.name}: ${response.status}`,
                );
                return null;
              }
          ...
          if (historicalRows.length > 0) {
            console.log(
              `✓ Loaded ${historicalRows.length} real historical data points`,
            );
  ...
  useEffect(() => {
    if (dataSource === "live" && isStreaming && activeMarket) {
      ...
      const interval = setInterval(() => {
        ...
        console.log(
          "📈 Sample prices:",
          Object.entries(currentPrices).slice(0, 3),
        );
    ...
    try {
      ...
      Papa.parse(content, {
        ...
        complete: (results) => {
          ...
          const dateIdxKey = firstRowKeys.find(
            (k) =>
          ...
          const processed = raw
            ...
            .filter((r) => !isNaN(r._ts as number))
  ...
  const handleTypeChange = (
    type: "price" | "spread" | "volatility" | "candlestick" | "heatmap",
  ) => {
    setChartOptions(null);
    setChartType(type);
  };
  ...
  const createChart = (
    ...
    if (type === "price") {
      ...
      const numBuckets = 20;
      const validSeries = Array.from(seriesMap.values()).filter(
        (pts) => pts.length > 0,
      );
      ...
      const endTime = Math.max(
        ...validSeries.map((pts) => pts[pts.length - 1][0]),
      );
      ...
      rangeSelector: {
        ...
        buttonTheme: {
          ...
          states: {
            ...
            select: { fill: "#4f46e5", style: { color: "white" } },
          },
        },
        ...
        <div className="flex items-center gap-6">
          <div>
            ...
            <p className="text-slate-400 text-sm">
              Visualize historical or live price data.
            </p>
          </div>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500 gap-4">
              {dataSource === "csv" ? (
                ...
                <>
                  ...
                  <p>
                    {isStreaming
                      ? "Listening for live data... chart will update shortly."
                      : "Stream is offline. Start a stream in the Scraper tab."}
                  </p>
                </>
              )}
            </div>
                    ...
                    <div className="space-y-2">
                      {(["sma", "macd", "rsi"] as const).map((ind) => (
                        ...
                        >
                          ...
                          />
                          <span className="text-xs font-medium text-slate-300 group-hover:text-white uppercase">
                            {ind}
                          </span>
                        </label>
                      ))}
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <select
                      value={spreadPair[0]}
                      onChange={(e) =>
                        setSpreadPair([e.target.value, spreadPair[1]])
                      }
                      className="w-full bg-slate-800 border border-slate-700 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      value={spreadPair[1]}
                      onChange={(e) =>
                        setSpreadPair([spreadPair[0], e.target.value])
                      }
                      className="w-full bg-slate-800 border border-slate-700 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                      <option value="">Select Asset</option>
                  ...
                  <p className="text-[10px] text-slate-500 leading-tight">
                    Calculates the absolute difference between the two selected
                    candidates.
                  </p>
                </div>
              ) : (
